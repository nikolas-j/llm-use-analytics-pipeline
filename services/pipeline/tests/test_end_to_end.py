...
def test_end_to_end_pipeline(temp_data_dir, monkeypatch):
    ...
    sanitized_file = temp_data_dir / "sanitized" / "date=2026-01-03" / "shard=00" / "messages.jsonl"
    ...
    # All conversations should be classified (even if as 'uncategorized')
...
def test_invalid_events(tmp_path, monkeypatch):
    ...
    assert report["conversations_assembled"] == 1
